...
[Series index](/blog/2026/harness-engineering-101/) · [Prev](/blog/2026/harness-04-agent-loop/) ·
[Next: Context Is a Budget](/blog/2026/harness-06-context-budget/)_
...
The work grows with the _square_ of the conversation length, because you pay
...
{% include figure.liquid loading="eager" path="assets/img/harness_cache_economics.png" class="img-fluid rounded z-depth-1" zoomable=true %}

_Generated by `diagrams/gen_cache_economics.py`._
...
- **A timestamp in the system prompt.** `Current time: 14:32:07` at the top
  of the array means _no request ever hits the cache_. If the model needs
...
- **Removing old messages from the middle** to save space. This is the
  painful one: trimming the array to make it _smaller_ can make it _more
  expensive_, because the trim invalidates the prefix. Context reduction has
...
whole steering mechanism on this principle, and it exists _because_ of this
chapter.
...
```json
{ "type": "text", "text": "...end of system prompt...", "cache_control": { "type": "ephemeral" } }
```
...
{% include figure.liquid loading="eager" path="assets/img/diagrams/harness-05-caching.svg" class="img-fluid rounded z-depth-1 diagram-img" zoomable=true %}
...
are seconds apart, so the cache stays hot. Where it bites is the _human_
...
- Memory and instructions ride in the _first user message_ in Claude Code
...
Part II is about what happens when the loop runs _long_: the array
approaches the context window, and everything from here on is about
spending that space well.

_[Next: Chapter 6 - Context Is a Budget, Not a Bag](/blog/2026/harness-06-context-budget/)_
