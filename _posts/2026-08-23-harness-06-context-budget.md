...
[Series index](/blog/2026/harness-engineering-101/) · [Prev](/blog/2026/harness-05-caching/) ·
[Next: Subagents](/blog/2026/harness-07-subagents/)_
...
{% include figure.liquid loading="eager" path="assets/img/harness_context_composition.png" class="img-fluid rounded z-depth-1" zoomable=true %}

_Illustrative composition of a long session. Generated by
`diagrams/gen_context_composition.py`._
...
- **Don't inject what the model can fetch.** The old instinct (from the
  ...
  _might_ need into the prompt up front. With tools, the model can pull what
  it _does_ need, when it needs it. Default to pull. This one sentence is
...
{% include figure.liquid loading="eager" path="assets/img/diagrams/harness-06-context-budget.svg" class="img-fluid rounded z-depth-1 diagram-img" zoomable=true %}
...
- **Inject memory as a user-side message, not into the system prompt.**
  ...
  this is material _about_ the user's world, on the user's channel.
- **Memory is a budget line too.** A CLAUDE.md that grows to 30,000 tokens
  ...
  and _points_ to deeper documents the model can read with tools when
...
no amount of trimming saves you. You need to spend _someone else's_ budget.
...
_[Next: Chapter 7 - Subagents: Fork the Context](/blog/2026/harness-07-subagents/)_
