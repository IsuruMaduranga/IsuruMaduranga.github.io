...
[Next: RAG](/blog/2026/harness-15-rag/)_
...
an invitation to transcription errors in the 900 lines it did _not_ mean to
change. Production harnesses use targeted edits: the model supplies an
exact existing snippet and its replacement, and the _harness_ refuses the
...
harmless error. The edit tool's design _is_ a guardrail; tool shape is
behavior shape.
...
each edit, ask the language server what is _newly_ broken, and inject the
...
that), but the harness _itself_ leaning on git: snapshot state so chapter
...
> the job needs, the models were _pre-trained on_ those tools' manuals and
...
Where it breaks down today, and notice each of these is a _body_ concern,
...
- **The feedback loops are real wins.** Nothing in the terminal _pushes_
...
- **Weaker brains need more body.** On frontier models, terminal-only is
  ...
  testing [One Code](/projects/one_code/) against cheap models) and structured tools with tight
...
behavior: every major one ships the shell _and_ the structured organs, and
...
- The terminal-only argument is half right: the shell is the irreplaceable
  ...
  undercounts what structure buys _now_: permissionability, pushed
...
_[Next: Chapter 15 - RAG Was a Harness Pattern All Along](/blog/2026/harness-15-rag/)_
